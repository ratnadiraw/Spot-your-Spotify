...
//                $save1[] = $row["song_id"];
                while($row = $result->fetch_assoc()) {
                    $tooltip = '<div style="padding:5px 5px 5px 5px;"><h4 style="margin-bottom:0; padding-bottom:0">'.str_replace("'","",utf8_encode ($row["title"])).'</h4><h5 style="margin-top:0; padding-top:0"><table> <tr> <td style="padding:5px 5px 5px 5px; margin-right: 5%">Danceability</td> <td><b>'.
                        ( $row["danceability"]).'</b></td> </tr><tr> <td style="padding:5px 5px 5px 5px;">Energy</td><td><b>'.
                        $row["energy"].
                        ' BPM</b></td> </tr> </table></h5></div>';
                    $resulta[]=array($row["danceability"], $row["energy"], $tooltip );
                }
